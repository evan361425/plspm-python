...
def test_plspm_russa():
    ...
    npt.assert_allclose(
        util.sort_cols(expected_scores), util.sort_cols(plspm_calc.scores())
    )

    expected_inner_model = pd.read_csv(
        "file:tests/data/russa.inner_model.csv", index_col=0
    )
    actual_inner_model = plspm_calc.inner_model()
    actual_inner_model = actual_inner_model[
        actual_inner_model["to"].isin(["POLINS"])
    ].drop(["to"], axis=1)
    ...
    expected_outer_model = pd.read_csv(
        "file:tests/data/russa.outer_model.csv", index_col=0
    )
    npt.assert_allclose(
        util.sort_cols(
    ...
    npt.assert_allclose(
        util.sort_cols(
            expected_crossloadings.filter(["AGRI", "IND", "POLINS"])
        ).sort_index(),
    ...
    npt.assert_allclose(
        util.sort_cols(expected_inner_summary.drop(["type"], axis=1)).sort_index(),
        util.sort_cols(
            plspm_calc.inner_summary().drop(["type", "r_squared_adj"], axis=1)
        ).sort_index(),
    )
    ...
    expected_outer_model_factorial = util.sort_cols(
        pd.read_csv(
            "file:tests/data/russa.outer_model_factorial.csv", index_col=0
        ).filter(["weight", "loading", "communality", "redundancy"])
    ).sort_index()
    npt.assert_allclose(
...
def test_plspm_russa_mode_b():
    ...
    config.add_lv(
        "POLINS", Mode.B, c.MV("ecks"), c.MV("demo"), c.MV("inst"), c.MV("death")
    )
...
def test_plspm_russa_categorical():
    ...
    plspm_calc = Plspm(russa, config, Scheme.CENTROID, 100, 0.0000001)
    expected_inner_summary = pd.read_csv(
        "file:tests/data/russa.categorical.inner_summary.csv", index_col=0
    )
    npt.assert_allclose(
        util.sort_cols(expected_inner_summary.drop(["type"], axis=1)).sort_index(),
        util.sort_cols(
...
def test_plspm_russa_categorical_mode_b():
    ...
    plspm_calc = Plspm(russa, config, Scheme.CENTROID, 100, 0.0000001)
    expected_inner_summary = pd.read_csv(
        "file:tests/data/russa.categorical.mode_b.inner_summary.csv", index_col=0
    )
    npt.assert_allclose(
        util.sort_cols(expected_inner_summary.drop(["type"], axis=1)).sort_index(),
        util.sort_cols(
