...
class Estimator:
    """Internal class that estimates the model. Designed to be thread-safe."""

    def __init__(self, config: c.Config):
        self.__hoc_path_first_stage = self.hoc_path_first_stage(config)

    def estimate(
        self, calculator: WeightsCalculatorFactory, data: pd.DataFrame
        ...
        # If we have higher order constructs, re-estimate the model using the
        # scores of the constituent LVs of the HOC generated by the first round
        # of estimation as the HOC's MVs.
        if hocs is not None:
            ...
            final_data, scores, weights = calculator.calculate(
                treated_data, config.path()
            )
    ...
    def hoc_path_first_stage(self, config: c.Config) -> pd.DataFrame:
        # For first pass, for HOCs we'll create paths from each and for each
        # exogenous LV to the HOC's constituent LVs, and from each consituent
        # LV to the endogenous LVs.
        path = config.path()
