    ...
    def summary(self):
        ...
        for lv in list(self.__config.path()):
            ...
            if not self.__data.loc[:, self.__config.mvs(lv)].isnull().values.any():
                mvs_for_lvs = (
                    util.treat(self.__data.filter(self.__config.mvs(lv)))
                    * self.__correction
                )
                pca_input = (
                    mvs_for_lvs
                    if mvs_for_lvs.shape[0] > mvs_for_lvs.shape[1]
                    else mvs_for_lvs.transpose()
                )
                pca = PCA()
                ...
                if self.__config.mode(lv) == Mode.A:
                    if mvs > 1:
                        ...
                        ca_denominator = (
                            pca_input.sum(axis=1).var() / self.__correction**2
                        )
                    ...
                    corr = np.corrcoef(
                        np.column_stack((pca_input.values, pca_scores[:, 0])),
                        rowvar=False,
                    )[:, -1][:-1]
                    rho_numerator = sum(corr) ** 2
                    rho_denominator = rho_numerator + (mvs - np.sum(np.power(corr, 2)))
                    summary.loc[lv, "dillon_goldstein_rho"] = (
