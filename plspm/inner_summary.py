...
class InnerSummary:
    """
    Internal class that computes a summary of the inner model.
    Use the methods :meth:`~plspm.Plspm.inner_summary` and :meth:`~plspm.Plspm.goodness_of_fit`
    defined on :class:`~.plspm.Plspm` to retrieve the inner model characteristics."""
    ...
    def __init__(
        ...
        block_communality = pd.Series(
            np.NaN, index=path.index, name="block_communality"
        )
        ...
        for lv in list(config.path()):
            ...
            mean_redundancy.loc[lv] = (
                outer_model.loc[:, "redundancy"].loc[config.mvs(lv)].mean()
            )
        ...
        if sum(num_mvs_in_lv) > 0:
            mean_communality = sum(
                x * y for x, y in zip(communality_aux, num_mvs_in_lv)
            ) / sum(num_mvs_in_lv)
            r_squared_aux = r_squared * lv_type
            self.__goodness_of_fit = np.sqrt(
                mean_communality * r_squared_aux[r_squared_aux != 0].mean()
            )
        else:
    ...
    def goodness_of_fit(self) -> float:
        ...
        if math.isnan(self.__goodness_of_fit):
            raise ValueError(
                "Cannot calculate goodness-of-fit if all constructs are single-item."
            )
