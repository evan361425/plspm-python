...
class _MetricWeights:
    ...
    def __init__(
        ...
        wf_diag = pd.DataFrame(
            np.diag(weight_factors),
            index=weight_factors.index,
            columns=weight_factors.index,
        )
    ...
    def iterate(self, inner_weight_calculator: Scheme) -> float:
        ...
        inner_weights = pd.DataFrame(
            inner_weight_calculator.value.calculate(self.__path, scores.values),
            index=lvs,
            columns=lvs,
        )
        ...
        for lv in list(lvs):
            ...
            weights = self.__config.mode(lv).value.outer_weights_metric(
                self.__data, z, lv, mvs
            )
            self.__weights.loc[mvs, [lv]] = weights
        weights_new = self.__weights.sum(axis=1).to_frame(name="weight")
        convergence = (
            np.power(self.__weights_old.abs() - weights_new.abs(), 2)
            .sum(axis=1)
            .sum(axis=0)
        )
    ...
    def calculate(self) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        weight_factors = 1 / (
            self.__data.dot(self.__weights).std(axis=0) / self.__correction
        )
        wf_diag = pd.DataFrame(
            np.diag(weight_factors),
            index=weight_factors.index,
        ...
        cor = (
            pd.concat([self.__data, scores], axis=1)
            .corr()
            .loc[list(self.__data), list(scores)]
        )
        ...
        if -1 in w_sign.values:
            ...
            w_sign_matrix = pd.DataFrame(
                np.diag(w_sign), index=w_sign.index, columns=w_sign.index
            )
            scores = scores.dot(w_sign_matrix)
        weights = pd.DataFrame(
            weights.sum(axis=1), index=self.__mvs, columns=["weight"]
        )
        return self.__data, scores, weights


class _NonmetricWeights:
    """Internal class that calculates weights and scores when using non-metric data."""
            ...
            if np.isnan(np.sum(mv_grouped_by_lv[lv])):
                ...
                for j in range(len(data.index)):
                    ...
                    denominator = np.power(
                        weight * self.__mv_grouped_by_lv_missing[lv][j, :], 2
                    ).sum()
                    if denominator == 0:
                        raise ValueError(
                            "All mvs for lv " + lv + " in row " + str(j) + " are NaN."
                        )
    ...
    def iterate(self, inner_weight_calculator: Scheme) -> float:
        ...
        inner_weights = inner_weight_calculator.value.calculate(
            self.__path, self.__scores
        )
        ...
        for i, lv in enumerate(list(self.__path)):
            for j, mv in enumerate(list(self.__config.mvs(lv))):
                self.__mv_grouped_by_lv[lv][:, j] = self.__config.scale(mv).value.scale(
                    lv, mv, z[:, i], self
                )
            self.__weights[lv], self.__scores[:, i] = self.__config.mode(
                lv
            ).value.outer_weights_nonmetric(
    ...
    def calculate(self) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        ...
        weight_factors = 1 / (
            data_new.dot(weights).std(axis=0, skipna=True) / self.__correction
        )
        ...
        return (
            data_new,
            pd.DataFrame(self.__scores, index=self.__index, columns=lvs),
            weights,
        )
    ...
    def get_z_for_mode_b(self, lv, mv, z_by_lv):
        ...
        return correction * (
            z_by_lv
            - np.delete(self.__mv_grouped_by_lv[lv], mv_index, axis=1).dot(
...
class WeightsCalculatorFactory:
    ...
    def clone(self):
        return WeightsCalculatorFactory(
            self.__config.clone(),
            self.__iterations,
            self.__tolerance,
            self.__correction,
    ...
    def calculate(self, data: pd.DataFrame, path: pd.DataFrame):
        ...
        if iteration > self.__iterations:
            raise RuntimeError(
                "Could not converge after " + str(iteration) + " iterations"
            )
